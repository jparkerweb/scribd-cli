 ...
 */
export async function isolateContainer(page: Page, selector: string): Promise<void> {
  await page.evaluate((sel) => {
    ...
    while (node && node.parentElement && node !== document.body) {
      ...
      for (const child of Array.from(parent.children)) {
        if (child !== node) (child as HTMLElement).style.display = 'none';
      }
  ...
  const dims = await page.evaluate((sel): PageDims | null => {
    const container = document.querySelector(sel) as HTMLElement | null;
    if (!container) return null;
    const first =
      (container.querySelector('.outer_page') as HTMLElement | null) ??
 ...
 */
export async function generatePdf(page: Page, outputPath: string): Promise<{ bytes: number }> {
