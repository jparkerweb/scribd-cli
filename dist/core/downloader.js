 ...
 */
export async function runDownload(opts, onProgress) {
    const emit = (stage, message, current, total) => onProgress({ stage, message, current, total });
    const debug = (...args) => {
        if (opts.debug)
            console.error('[scribd-cli]', ...args);
    };
 ...
 */
function resolveOutputPath(outputDir, fileName, id) {
 ...
 */
async function navigateWithRetry(page, url, debug) {
    let lastErr;
 ...
 */
async function scrollLoad(page, emit, debug) {
    ...
    while (iter < maxIterations && Date.now() - start < maxTime) {
        ...
        const measure = await page.evaluate((sel) => {
            ...
            const isLoaded = (p) => p.querySelectorAll('img,canvas,svg').length > 0 || (p.textContent ?? '').trim().length > 50;
        ...
        iter++;
        debug(`scroll iter ${iter}: loaded=${measure.loaded}/${measure.total} atBottom=${step.atBottom} noGrowth=${noGrowth}`);
