...
export class ScribdCliError extends Error {
    ...
    friendlyMessage;
    constructor(message, exitCode, friendlyMessage) {
        super(message);
...
export class BrowserError extends ScribdCliError {
    constructor(message = 'Failed to launch headless browser') {
        super(message, 6, "Couldn't start the headless browser. Try `npx playwright install chromium`.");
    }
}
/** Catch-all for anything not modeled above (exit 1). */
export class UnknownError extends ScribdCliError {
    constructor(cause) {
        super(cause instanceof Error ? cause.message : String(cause), 1, 'An unexpected error occurred.');
    }
}
